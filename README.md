...
- 普通“总结经验 / 工作流 / 下次按这个做”只作为参考记忆，不直接生成经验候选
- 只有用户明确要求“固化为经验 / 沉淀为经验 / 形成经验候选”时，才会额外生成 `WORKFLOW` 学习候选
- 只有用户明确要求“记录为教训 / 沉淀教训 / 避免再犯”时，才会额外生成 `NEGATIVE_LESSON` 学习候选
- 单次成功任务不会自动生成正向经验；正向经验需要多次复用验证后再沉淀
- 单次相似成功流程会作为运行时参考注入，不作为正式经验
- 同类流程默认成功 3 次且置信度足够后，会自动沉淀为 `WORKFLOW_EXPERIENCE`
