...
- 可恢复执行：规划任务或长任务因网络、provider、工具异常失败后，可在同一会话输入“继续 / 重试”从最近 checkpoint 恢复。
...
JobClaw 会在计划初始化、步骤推进、失败分类和失败结束时保存任务 checkpoint。

- 同一会话中输入 `继续`、`继续执行`、`恢复执行`、`重试`、`continue`、`resume` 或 `retry` 时，会优先恢复最近的可恢复 checkpoint。
- 恢复时使用 checkpoint 中保存的原始任务、规划模式、plan execution snapshot 和子任务状态，而不是把“继续”当成一个新的独立任务。
- 已完成步骤、已完成子任务和已存储的大结果会作为恢复提示注入，避免重复执行已完成工作。
- 网络、provider I/O、临时工具失败等非业务失败可以通过继续执行恢复；真正缺配置或不可恢复的问题仍会进入 `BLOCKED / FAILED`。
...
聊天页会把长任务运行事件整理成统一运行视图，而不是把所有事件混在普通对话流中：

- 总任务情况：当前阶段、完成判定、待处理子任务、修复次数、运行中/完成/异常计数。
- 规划流程：计划初始化、任务路由、计划复核、步骤完成和完成判定。
- 主智能体流程：主智能体的工具调用集中展示，可展开查看参数和结果。
- 子智能体流程：每个子智能体独立成卡片，包含子任务事件和子智能体内部工具调用。
- 对话内容：保留用户消息和最终回复；工具事件、harness 事件和子智能体事件进入运行视图。

回复消息支持一键复制。点击复制后，按钮会短暂切换为“已复制”，并通过全局 toast 给出反馈。
